...
namespace Monitor
{
    public class reqSuprimento
    {
        ...
        {
            string tsql = "select postagem from enviosSuprimentos where dtEntrega is null and dtEnvio > getdate()-90 and tpenvio in ('SEDEX','PAC')";
            List<string> listaPostagens = new List<string>();
            DataTable dtPostagens = DAO.retornaDt(tsql);
            foreach (DataRow drPostagen in dtPostagens.Rows)
            {
                listaPostagens.Add(drPostagen["postagem"].ToString());
            }
            return listaPostagens;
        }

        internal static bool Entregue(string data, string postagem)
        {
            string tsqlUpdate = string.Format("update enviosSuprimentos set statusEntrega = 'Objeto entregue ao destinatário', dtEntrega = '{0}' where postagem = '{1}';", data, postagem);
        ...
        internal bool AtualizarStatus(string descricao, string data)
        {
            bool result = false;
            string tsqlUpdate = null;

            tsqlUpdate = string.Format("update enviossuprimentos set statusEntrega = '{0}', dtEntrega = '{2}' where idEnvio = {1};", descricao, this.Id, data);
            result = DAO.Execute(tsqlUpdate);
            return result;
        }
    }
