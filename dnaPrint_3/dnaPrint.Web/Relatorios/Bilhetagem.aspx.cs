...
namespace dnaPrint.Web.Relatorios
{
    public partial class Bilhetagem : System.Web.UI.Page
    {
        ...
        private static IOrderedEnumerable<Base.Bilhetagem> listaBilhetagem = null;
        ...
        protected void tbExibir_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbdtInicial.Text) && !string.IsNullOrEmpty(tbdtFinal.Text))
            {
                ...
                listaBilhetagem = Base.Bilhetagem.Listar(Session["ConnString"].ToString(), dnaPrint.DAO.Operacoes.DefinirTipo(Session["TipoDB"].ToString()), dtInicial, dtFinal).OrderBy(x => x.Unidade);
                

                if (listaBilhetagem.Count() > 0)
                {
                    gvBilhetagem.DataSource =
                listaBilhetagem;
        ...
        protected void tbExportar_Click(object sender, EventArgs e)
        {
            ...
            ReportDataSource ds = new ReportDataSource("dsBilhetagem", listaBilhetagem);
