    ...
    public partial class Equipamentos : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                tbFila.Attributes.Add("placeholder", Equipamento.ProximoFila(Session["ConnString"].ToString()
                    , dnaPrint.DAO.Operacoes.DefinirTipo(Session["TipoDB"].ToString())).ToString());
            }
        }
        ...
        protected void tbAdicionar_Click(object sender, EventArgs e)
        {
            ...
            if (!serieExiste)
            {
                if (!filaExiste)
                {
                    if (eqp.Adicionar(Session["ConnString"].ToString()
                        , dnaPrint.DAO.Operacoes.DefinirTipo(Session["TipoDB"].ToString())))
                    {
                        LimparCampos();
                    }
                }
        ...
        protected void TbSalvar_Click(object sender, EventArgs e)
        {
            ...
            eqp.IP = tbIP.Text;
            eqp.nome = tbFila.Text;
            eqp.idModeloEquipamento = int.Parse(dpModeloEqpto.SelectedItem.Value);
